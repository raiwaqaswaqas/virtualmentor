...
include("include/user_data.php");
	...
	$run_match = mysqli_query($con, "SELECT * from library where category='$category'");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body spellcheck="true">
	...
	<div class="container my-5">
		<h2 class="text-center text-capitalize text-dark"><?php echo $category;?> Books</h2><hr class="w-50">
	...
	<!-- Start book display area -->
		...
		<?php
			if ($run_match) {
				while ($result=mysqli_fetch_assoc($run_match)) {
					echo"
					<div class='col-md-4 my-3'>
						<div class='card' style='width: 16rem;'>
							<img src='admin/books/images/".$result['image']."' height='250px'>
							<div class='card-body'>
								<h5 class='card-title text-capitalize text-truncate mt-n2 pb-1'>".$result['title']."</h5><hr class='mt-n1'>
								<p class='card-text small overflow-hidden text-ellipsis mt-n2' style='height:40px;'>".$result['description']."</p>
								<a href='admin/books/".$result['file']."' target='_blank' class='btn btn-outline-info btn-sm'>View</a>
								<a href='admin/books/".$result['file']."' download='".$result['file']."' class='btn btn-outline-secondary btn-sm'>Download</a>
								<a href='include/add-book-pl.php?id=".$result['id']."' class='btn btn-outline-secondary btn-sm'>Add in PL</a>
							</div>
						</div>
					</div>
					";
